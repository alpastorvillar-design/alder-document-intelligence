...
class TestAFindingsFactsAreSizedByWhatIsInThem:
    ...
    Measured after: each pair as wide as what it holds, packed left, and the
    URL on one line. The box itself is full width again and the block is as
    wide as the screen - both were narrowed for a round and reverted, because
    an inset column read worse against the sections above and below it than
    the space it saved.
    """
    ...
    def test_the_block_can_shrink_instead_of_being_cut_off(self) -> None:
        """Insurance against a trap this screen has already fallen into.

        A flex item does not shrink below its own content unless told to. When
        the facts box asked for `max-content` that meant it kept its widest
        size on a narrow window and `.finding { overflow: hidden }` cut it off
        - at 375px the figures were gone rather than wrapped, 1048px clipped.
        The box is full width again so the trap is not currently armed; the
        declaration stays because the clipping half of it does too.
        """
